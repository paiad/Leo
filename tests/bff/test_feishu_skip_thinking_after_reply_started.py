...
@pytest.mark.asyncio
async def test_feishu_skips_thinking_after_reply_chunks_started(monkeypatch):
    ...
    async def fake_stream_message(_payload):
        yield "event: progress\ndata: " + json.dumps(
            {"phase": "thinking", "message": "准备开始"}, ensure_ascii=False
        ) + "\n\n"
        yield "event: chunk\ndata: " + json.dumps({"content": "最终"}, ensure_ascii=False) + "\n\n"
        # A late thinking event should be ignored once chunks started.
        yield "event: progress\ndata: " + json.dumps(
            {"phase": "thinking", "message": "这条不该发"}, ensure_ascii=False
        ) + "\n\n"
        yield "event: chunk\ndata: " + json.dumps({"content": "总结"}, ensure_ascii=False) + "\n\n"
