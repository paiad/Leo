...
@pytest.mark.asyncio
async def test_feishu_sends_thoughts_progress_and_final_summary(monkeypatch):
    ...
    async def fake_stream_message(_payload):
        yield "event: progress\ndata: " + json.dumps(
            {"phase": "thinking", "message": "好的！Leo 来帮你查抖音 top5 热点新闻！啾～"},
            ensure_ascii=False,
        ) + "\n\n"
        yield "event: progress\ndata: " + json.dumps(
    ...
    assert sent == ["好的！Leo 来帮你查抖音 top5 热点新闻！啾～", "最终总结"]
