...
@pytest.mark.asyncio
async def test_planning_orchestrator_falls_back_when_planner_json_invalid(monkeypatch):
    ...
    async def _invalid_plan(prompt, prefilter):
...
@pytest.mark.asyncio
async def test_planning_orchestrator_returns_no_mcp_for_meta_query(monkeypatch):
    store = InMemoryStore(enable_persistence=False)
