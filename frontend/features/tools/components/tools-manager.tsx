...
export function McpManager() {
  ...
  return (
    <div className="space-y-6 rounded-[24px] border border-slate-200 bg-white p-8">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">MCP</h1>
        <p className="mt-2 text-sm text-slate-600">
          粘贴 <code>mcpServers</code> JSON 并导入。页面仅做导入和展示，不包含 discover。
        </p>
      </div>

      <section className="rounded-xl border border-slate-200 p-4">
        <h2 className="text-sm font-semibold text-slate-900">导入 JSON</h2>
        <textarea
          ref={importTextareaRef}
          value={importJson}
          onChange={(event) => setImportJson(event.target.value)}
          className="mt-3 w-full overflow-hidden rounded-lg border border-slate-200 px-3 py-2 font-mono text-xs outline-none focus:border-slate-400"
      ...
      <section className="rounded-xl border border-slate-200 p-4">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-900">已添加 MCP ({servers.length})</h2>
          <button
            type="button"
        ...
        <ul className="mt-3 space-y-3">
          {servers.map((server) => (
            <li key={server.serverId} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-red-300 text-red-600 disabled:opacity-60"
          ...
          {servers.length === 0 ? (
            <li className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-xs text-slate-500">
