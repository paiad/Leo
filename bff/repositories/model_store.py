...
class ModelPostgresStore:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    UPDATE workspace_models
                    SET name = %s, provider = %s, base_url = %s, api_key = %s, enabled = %s, updated_at = %s
                    WHERE id = %s
                    RETURNING id, name, provider, base_url, api_key, enabled, created_at, updated_at
                    """,
                    (
                        payload["name"],
                        payload["provider"],
    ...
    def delete_model(self, model_id: str) -> bool:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM workspace_models WHERE id = %s", (model_id,))
                deleted = cur.rowcount > 0
                cur.execute(
                    "DELETE FROM workspace_settings WHERE key = 'active_model_id' AND value = %s",
                    (model_id,),
                )
            conn.commit()
        return deleted
    ...
    def get_active_model_id(self) -> str | None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT value FROM workspace_settings WHERE key = 'active_model_id'")
                row = cur.fetchone()
        if row is None:
            return None
        return row["value"]
    ...
    def set_active_model_id(self, model_id: str | None) -> None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                if model_id is None:
                    cur.execute("DELETE FROM workspace_settings WHERE key = 'active_model_id'")
                else:
                    cur.execute(
                        """
                        INSERT INTO workspace_settings (key, value)
                        VALUES ('active_model_id', %s)
                        ON CONFLICT(key) DO UPDATE SET value = EXCLUDED.value
                        """,
                        (model_id,),
                    )
...
def create_model_store() -> ModelSqliteStore | ModelPostgresStore:
    database_url = os.getenv("BFF_DATABASE_URL", "").strip() or os.getenv("DATABASE_URL", "").strip()
    if database_url.startswith("postgresql://") or database_url.startswith("postgres://"):
        return ModelPostgresStore(database_url=database_url)
    return ModelSqliteStore()
